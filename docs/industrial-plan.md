...
- **AI 対話による機能作成**（2026-08-26 議論）: タグ定義（I1）・
  トレンドのグループ表示構成（R1 — 表示は固定グループパターンのみ、
  の護りと整合）・帳票テンプレート（R3。`@banto/report` はテンプレートを
  文字列 prop で受けるため実行時生成に対応済み）は
  **シリアライズ可能な DB データとして持つ**。
  ...
  （監査付き）の関所を通す。汎用画面エディタは引き続き R4 の護りの
  とおり v1 で入れない — 固定グループパターンの語彙拡張で賄う。
  R0 要件定義時にこの設計方向を前提に含める。
