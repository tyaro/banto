...
- SSE（`/api/events`）は接続時に照合し、開いている間も keepalive と同じ間隔
  ...
  を既知の制約としていた）。失効から通知が止まるまで最大 1 間隔（と照合 1 回分）の
  遅れがある。照合の判断は要求と同じ（`authenticate`。照合中に自分の変更で
  付け替えられたセッションは残し、DB が答えられなければストリームを残して次の
  間隔でやり直す）。照合は `REVALIDATE_TIMEOUT`（5 秒、間隔より短い）で打ち切り、
  打ち切りも「照合できなかった」としてストリームを残す（打ち切った読み取りは
  破棄され、あとからセッションを変えない）。次の照合の期限は照合が**終わった時点**
  から 1 間隔後に置くので、照合が遅くても間隔ごとに通知を配る機会が必ずある
  （#234 のレビュー対応。期限を照合の開始から数えると、照合が間隔より長いときに
  照合だけが続いて通知が止まる）。この照合は無操作期限（idle）を延ばさない
  （開いたタブが無操作のセッションを生かし続けない）。期限はストリーム 1 本に 1 つで、
  ストリーム自身が持つ（タスクを起こさない）ため、切断で期限も照合中の読み取りも
  一緒に破棄される。同じプロセスでログアウト・失効を知った時点で即座に閉じる仕組み
...
- フロント（`@banto/admin-core`）は、失効が**確認できた**ときだけトークンを消し、
  ...
  覚えておき、次の購読で `check()` により確認し直してから知らせる（新しいログインを
  古い失効で追い出さないため、再送ではなく確認し直す）。照合
  できない（`500`・到達不能・10 秒応答なし）確認は、SSE の再送は止めたまま、
  確認だけを 1 秒から倍々（上限 30 秒）で再試行する。10 秒を過ぎて届いた
  `false` はトークンを消し、次の再試行がトークン無しで失効を確定して知らせる。
  別のタブが Remember me の共有トークンを先に消したときは、SSE が再接続で
  「使っていたトークンが消えた」ことを検知して同じ確認に渡す（初回のログイン前の
  待機・新しいログインによる置き換えは除く。#242 のレビュー対応）。アプリはそこでルートガードを再実行する
...
- `AuthState::revalidate` は `pub` にして派生アプリへ公開している（Issue #239、
  ...
  無操作期限だけ延ばさずに再検証できるようにするため。判断は `authenticate` と
  同一（本節の再検証と同じ）で、呼び出し方（周期・タイムアウト・期限の起点）は
  この節の `/api/events` の実装（`events.rs`）を手本にするようドキュメントで
  案内している。
